...
- **뮤직잇츠 서명** — 모든 커버 오른쪽에 작게 들어갑니다. 배경이 밝으면 검정, 어두우면 흰색으로 **알아서 바뀝니다**
...
화면은 **왼쪽이 미리보기, 오른쪽이 설정판**입니다 (폰 목업 스튜디오와 같은 배치).

1. 오른쪽 설정판 **1. 사진**에 끌어다 놓거나 눌러서 고릅니다 (여러 장 가능)
2. **3. 제호**에서 매거진을 고릅니다
3. 문구를 내 것으로 바꿉니다 — 줄 앞에 `*`를 붙이면 그 줄만 강조색
4. **13. 내보내기**에서 PNG로 저장
...
- **미리보기는 화면 위에 붙어 있습니다.** 설정을 아래로 넘겨도 커버가 계속 보여서, 슬라이더를 만지면서 바로 확인할 수 있어요
- 설정은 `1. 사진` 부터 `13. 내보내기` 까지 한 줄로 이어집니다. 제목 줄을 누르면 접힙니다
- 사진이 두 장 이상이면 미리보기 아래에 **작은 사진 줄**이 생깁니다. 커버 설정은 그대로 둔 채 사진만 바꿉니다
- 맨 아래 **[사진 선택]** · **[저장]** — `저장`은 휴대폰 공유창을 띄워 사진 앨범·메신저로 바로 보냅니다
...
- 미리보기의 제호·문구를 **톡** 치면 그 항목으로 바로 내려갑니다
